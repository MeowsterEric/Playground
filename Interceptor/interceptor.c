...
typedef struct {
	...
	int monitored;
	/* List of monitored PIDs when monitored=1, or list of non-monitored PIDs when monitored=2*/
	int listcount;
...
/**
 * Check if a pid is in the the pid_list for the given system call.
 * Returns 1 if it already is, or 0 if pid is not in sysc's list.
 */
static int check_pid_in_list(int sysc, pid_t pid) {
...
/**
 * Replaces the system call pointed by origin with repl,
 * the sys_call_table must be writable before calling this function.
 * Requires calltable_lock.
 * Returns the original system call.
 */
static void *repl_sysc(int syscall, void *repl) {
	void *orig = sys_call_table[syscall];
	sys_call_table[syscall] = repl;
	return orig;
}

/**
 * Returns whether the given pid is being monitored explicitly/implicitly.
 * Requires pidlist_lock.
 * Requires pid to be an existing pid. (not the pid 0)
 */
static int is_pid_monitored_effectively(int syscall, pid_t valid_pid) {
	return check_pid_in_list(syscall, valid_pid) ?
	       table[syscall].monitored == 1 :
	       table[syscall].monitored == 2;
}

/**
 * Returns true iff all pids are being monitored (no pids in blacklist)
 * Requires pidlist_lock.
 */
static int are_all_pids_monitored(int syscall) {
	return table[syscall].monitored == 2 && list_empty(&table[syscall].my_list);
}

/**
 * Returns true iff at least one pid is monitored
 * Requires pidlist_lock.
 */
static int is_any_pid_monitored(int syscall) {
	return table[syscall].monitored == 2 || !list_empty(&table[syscall].my_list);
}
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	// retrieve original system call
	f = table[reg.ax].f;
	spin_unlock(&calltable_lock);

	spin_lock(&pidlist_lock);
	// log a message if the calling pid is being monitored
	if (is_pid_monitored_effectively(reg.ax, current->pid))
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	// delay the return of error code if necessary
	int error = 0;
	...
	// validation for the last two commands
	if (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING) {
		// pid must be 'all pids' or an existing pid
		if (pid != PID_ALL && !pid_task(find_vpid(pid), PIDTYPE_PID))
			return -EINVAL;

		// caller must be root or the owner of the given process
		if (current_uid() != ROOT_USER) {
			if (pid == PID_ALL || check_pid_from_list((pid_t)pid, current->pid) != 0)
				return -EPERM;
		}
	}
	...
	case REQUEST_SYSCALL_INTERCEPT:
		...
		if (table[syscall].intercepted) {
			...
			// replace the given system call with our interceptor
			set_addr_rw((unsigned long)sys_call_table);
			table[syscall].f = repl_sysc(syscall, interceptor);
	...
	case REQUEST_SYSCALL_RELEASE:
		...
		if (!table[syscall].intercepted) {
			...
			// restore to the original system call
			set_addr_rw((unsigned long)sys_call_table);
			repl_sysc(syscall, table[syscall].f);
			set_addr_ro((unsigned long)sys_call_table);
			table[syscall].f = NULL;
			table[syscall].intercepted = 0;
			// clear the monitored pids for the given system call
			spin_lock(&pidlist_lock);
	...
	case REQUEST_START_MONITORING:
		...
		// cannot start monitoring if
		//   - syscall has not been intercepted yet, or
		//   - pid is being effectively monitored
		if (!table[syscall].intercepted)
			error = -EINVAL;
		else if (pid == PID_ALL ?
		         are_all_pids_monitored(syscall) :
		         is_pid_monitored_effectively(syscall, pid))
			error = -EBUSY;
		if (!error) {
			if (pid == PID_ALL) {
				destroy_list(syscall);
				...
				if (table[syscall].monitored == 2) {
					// remove monitored pid from blacklist
	...
	case REQUEST_STOP_MONITORING:
		...
		// must have syscall intercepted and,
		// - if stop for all, then at least one pid must have been monitored
		// - if stop for one, then it must be monitored effectively
		if (!table[syscall].intercepted)
			error = -EINVAL;
		else if (pid == PID_ALL ?
		         !is_any_pid_monitored(syscall) :
		         !is_pid_monitored_effectively(syscall, pid))
			error = -EINVAL;
...
/**
 * Stores original custom_syscall function
 */
...
static int init_function(void) {
	int i;
	spin_lock(&calltable_lock);
	// Hijack custom_syscall and exit_group
	set_addr_rw((unsigned long)sys_call_table);
	orig_custom_syscall = repl_sysc(MY_CUSTOM_SYSCALL, my_syscall);
	orig_exit_group = repl_sysc(__NR_exit_group, my_exit_group);
	set_addr_ro((unsigned long)sys_call_table);

	// Initialize pid lists
	spin_lock(&pidlist_lock);
...
static void exit_function(void)
{
	...
	// Restore all intercepted syscalls
	for (s = 1; s < NR_syscalls; s++) {
		if (table[s].intercepted)
			repl_sysc(s, table[s].f);
	}

	// Restore the special custom_syscall and exit_group
	repl_sysc(MY_CUSTOM_SYSCALL, orig_custom_syscall);
	repl_sysc(__NR_exit_group, orig_exit_group);
